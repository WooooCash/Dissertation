...
\label{tab:prz}
\end{table}
Przykład Tabeli~\ref{tab:prz} został zaczerpnięty ze strony \cite{przyklad}. Tak właśnie odwołujemy się do~tabel.%~ zapobiega łamaniu wierszy w danym miejscu
\subsection{Rysunki}
...
\section{Listingi}
Korzystając ze środowiska listings możemy formatować listingi.
...
Schematy wykonujemy przy użyciu środowiska tikz \footnote{Przykład zaczerpnięty ze strony \cite{przyklad2}}:
\begin{figure}[ht]
\centering
\begin{tikzpicture}
...
Do składania prac dyplomowych w środowisku Windows polecam edytor \href{http://www.latexeditor.org}{LED} wraz z~kompilatorem  \href{http://miktex.org/}{Mik\TeX}. Wszystkie potrzebne informacje dotyczące systemu \LaTeX ~można znaleźć w \cite{lshort,e-do-pi,pgfman,grafika,gust}. Zbiór klas \cite{przyklad3}.
