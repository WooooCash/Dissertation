...
ray cast logic, and \textbf{CHECK BACK LATER}.

\subsubsection{Ray casts}
The ray cast objects contain a script component which dispatches the rays and
sets the appropriate target positions. First, a mask must established, which
will then be passed into the ray cast operation. This is required so that only
terrain is counted as a valid hit. The lack of such mask may result in
unexpected behaviour, such as targets being set on the spiders body itself. The
...
are sent towards the surface that the spider is standing on.
